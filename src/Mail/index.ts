...
export const htmlWelcome = ({ userName, token }: props) => {
    if (!userName) throw new Error("userName is required");
    if (!token) throw new Error("token is required");
    const WelcomeTemplate = fs.readFileSync(
        "./Mail/WelcomeTemplate.html",
        "utf8"
    );

    const modifedWelcome = WelcomeTemplate.replace(
        /@userName/g,
        userName
...
export const htmlSerieUpdated = async ({ serieName, imgURL }: props) => {
    ...
    const modifedSerie = SerieUpdated.replace(/@serieName/g, serieName).replace(
        /@imgURL/g,
        imgURL
    );
...
export const htmlTemporada = ({
    ...
    if (!imgURL) throw new Error("imgURL is required");
    const TemporadaUpdated = fs.readFileSync(
        "./Mail/TemporadaUpdated.html",
        "utf8"
    );

    const modifedTemp = TemporadaUpdated.replace(/@serieName/g, serieName)
        .replace(/@numCap/g, numCap)
        .replace(/@numTemp/g, numTemp)
